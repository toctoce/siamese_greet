...
def train_discriminator(cl_model0, cl_model1, discriminator0, discriminator1, optimizer_disc, features, str_encodings, edges, args, flag):
    ...
    if flag:
        embedding = discriminator1.emb_layer(embedding)
    else:
        embedding = discriminator0.emb_layer(embedding)
...
def main(args):
    ...
    for trial in range(args.ntrials):
        ...
        discriminator0 = Edge_Discriminator(nnodes, nfeats, args.alpha, args.sparse, args.emb_dim).to(device)
        discriminator1 = Edge_Discriminator(nnodes, str_encodings.shape[1], args.alpha, args.sparse, args.emb_dim).to(device)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            rank_loss0 = train_discriminator(cl_model0, cl_model1, discriminator0, discriminator1, optimizer_discriminator0, features, str_encodings, edges, args, 0)
            rank_loss1 = train_discriminator(cl_model0, cl_model1, discriminator0, discriminator1, optimizer_discriminator1, features, str_encodings, edges, args, 1)
            ...
            if epoch % args.eval_freq == 0:
                ...
                # 1. 그냥 concat한 임베딩으로 테스트
                # 2. 각 ED로 임베딩 만든 후 평균 or concat
                e0 = discriminator0.emb_layer(embedding)
                e1 = discriminator1.emb_layer(embedding)

                embedding = torch.cat((e0, e1), dim=1)
                # 3. 각 임베딩(embedding0, embedding1)과 각 ED로 임베딩 생성 후 평균 or concat
                #   (하지만 이 부분은 임베딩 크기가 embedding의 절반이므로 레이어 shape가 다른 점 인지.)
                # ...
